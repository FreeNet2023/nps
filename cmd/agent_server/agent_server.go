...
func main() {
	...
	if err := beego.LoadAppConfig("ini", filepath.Join(common.GetRunPath(), "conf", "agent_server.conf")); err != nil {
	...
	if len(os.Args) > 1 && os.Args[1] != "service" {
		switch os.Args[1] {
		case "reload":
			daemon.InitDaemon("agent_server", common.GetRunPath(), common.GetTmpPath())
			return
		case "install":
			// uninstall before
